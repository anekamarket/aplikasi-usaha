...
const DEFAULT_TELLER = { id: 'U002', username: 'teller', password: CryptoJS.SHA256('teller123').toString(), name: 'Teller', role: 'teller' };

let users, members, settings, savings, loans, pawns, transactions, currentUser;
...
function initializeData() {
    ...
    if (!users_init.find(u => u.username === 'teller')) users_init.push(DEFAULT_TELLER);
    ...
    if (!localStorage.getItem(STORAGE_KEYS.SETTINGS)) {
        const initialSettings = {
            koperasiName: 'KOPERASI DIGITAL', koperasiAddress: 'Jl. Digital No. 1, Kota Maya',
            koperasiPhone: '081234567890', koperasiEmail: 'info@koperasi.com',
            panelSecurityCode: CryptoJS.SHA256('LKS.1945').toString(), // Default password
            roles: {
                admin: { label: "Admin", isEditable: false, permissions: getPermissionKeys(true) },
                manager: { label: "Manajer", isEditable: true, permissions: {...getPermissionKeys(false), canManageLoans: true, canManagePawns: true, canViewReports: true, canAccessControlPanel: true } },
                teller: { label: "Teller", isEditable: true, permissions: {...getPermissionKeys(false), canManageSavings: true, canManageMembers: true, canReceiveInstallments: true } }
            }
        };
...
$(document).ready(function() {
    ...
    // Savings
    $('#savingsMemberSearch').on('input', function() { handleMemberSearch(this, '#savingsMemberResults', selectMemberForSavings); });
    ...
    $('#loanMemberSearch').on('input', function() { handleMemberSearch(this, '#loanMemberResults', selectMemberForLoan); });
    $('#loanAmount, #loanTenor, #loanInterest').on('input', calculateLoanPreview);
    $('#saveLoanBtn').click(saveLoan);
    $(document).on('click', '.view-loan-details', function() { openLoanDetailsModal($(this).data('id')); });
    ...
    $('#pawnMemberSearch').on('input', function() { handleMemberSearch(this, '#pawnMemberResults', selectMemberForPawn); });
    $('#savePawnBtn').click(savePawn);
    $(document).on('click', '.redeem-pawn-btn', function(e) { e.stopPropagation(); redeemPawn($(this).data('id')); });
    ...
    $('#manageMemberSearchInput').on('input', function() { loadMembersList($(this).val()); });
    ...
    $(document).on('click', '.profile-member', function() { openMemberProfileModal($(this).data('id')); });
    ...
    // Data Management
    $('#backupDataBtn').click(() => showConfirmation('Backup Data', 'Data akan di-backup ke file terenkripsi (.lkscoop). Lanjutkan?', 'success', backupData));
    $('#restoreFile').change(function() { promptForPassword('panel', () => { restoreFromFile(this.files[0]); $(this).val(''); }); });
    $('#resetDataBtn').click(() => promptForPassword('panel', () => showConfirmation('Reset Data', 'Semua data akan dihapus permanen. Lanjutkan?', 'danger', resetAllData)));

    // Password Prompt
    $('#passwordPromptForm').submit(e => {e.preventDefault(); submitPasswordPrompt();});
...
function initializeApp() {
    updateKoperasiInfo();
    configureUIAccess();
    updateLoanStatuses(); // Check for overdue loans on app start
    loadDashboard();
    updateCurrentDate();
}
...
function loadDashboard() {
    ...
    const totalActiveLoans = loans.filter(l => l.status === 'active' || l.status === 'overdue').reduce((sum, l) => sum + l.outstandingPrincipal, 0);
    const totalAssets = totalSavings; // Simplified for now
...
function handleMemberSearch(inputElement, resultsElement, callback) {
    ...
    const filteredMembers = members.filter(m => m.name.toLowerCase().includes(searchTerm) || m.id.toLowerCase().includes(searchTerm));
    $results.empty().show();
    if (filteredMembers.length > 0) {
        filteredMembers.forEach(m => $results.append(`<div class="member-search-item" data-id="${m.id}">${m.name} (${m.id})</div>`));
        $results.off('click').on('click', '.member-search-item', function() {
            const memberId = $(this).data('id');
            const memberName = $(this).text();
            $(inputElement).val(memberName);
            $(inputElement).data('selected-id', memberId);
            $results.hide();
...
function resetMemberForm() { $('#memberForm')[0].reset(); $('#memberIdInput').val(''); $('#memberFormTitle').text('Tambah Anggota Baru'); }
...
function saveMember(e) {
    ...
    const newMemberData = { id, name: $('#memberName').val(), phone: $('#memberPhone').val(), address: $('#memberAddress').val() };

    const existingMemberIndex = members.findIndex(m => m.id === id);
    if(existingMemberIndex > -1) {
        members[existingMemberIndex] = { ...members[existingMemberIndex], ...newMemberData };
        showToast('Data anggota berhasil diperbarui', 'success');
    } else {
        newMemberData.joinDate = new Date().toISOString();
...
function loadMemberSavingsSummary(memberId) {
    ...
    const totalPokok = memberSavings.filter(s => s.category === 'pokok').reduce((sum, s) => sum + s.amount, 0);
    const totalWajib = memberSavings.filter(s => s.category === 'wajib').reduce((sum, s) => sum + s.amount, 0);
    const totalSukarela = memberSavings.filter(s => s.category === 'sukarela').reduce((sum, s) => sum + s.amount, 0);
    $('#summaryPokok').text(formatCurrency(totalPokok));
    $('#summaryWajib').text(formatCurrency(totalWajib));
    $('#summarySukarela').text(formatCurrency(totalSukarela));
    $('#summaryTotalSimpanan').text(formatCurrency(totalPokok + totalWajib + totalSukarela));
}
...
function processSavingsTransaction(e) {
    ...
    const memberSavings = savings.filter(s => s.memberId === memberId && s.category === category);
    const currentBalance = memberSavings.reduce((sum, s) => sum + s.amount, 0);

    if(type === 'withdrawal' && amount > currentBalance) return showToast(`Saldo ${category} tidak mencukupi untuk penarikan.`, 'error');

    if(type === 'withdrawal') amount = -amount;

    const newTransaction = { id: 'SAV' + moment().format('x'), memberId, type, category, amount, date: new Date().toISOString(), processedBy: currentUser.name };
...
function loadLoansList() {
    ...
    if (loans.length === 0) return $list.html('<div class="empty-state"><p>Belum ada data pinjaman.</p></div>');
    ...
    const sortedLoans = [...loans].sort((a, b) => {
        const statusOrder = { 'overdue': 1, 'active': 2, 'paid_off': 3 };
    ...
    sortedLoans.forEach(loan => {
        ...
        let statusClass = '';
        if(loan.status === 'paid_off') statusClass = 'text-success';
        else if(loan.status === 'overdue') statusClass = 'text-danger';
...
function saveLoan() {
    ...
    if (!memberId || !(amount > 0) || !(tenor > 0) || isNaN(interestRate)) return showToast('Harap lengkapi semua data pinjaman dengan benar.', 'error');
    ...
    const newLoan = {
        ...
        memberId, amount, tenor, interestRate, collateral,
        monthlyInstallment,
...
function loadPawnList() {
    ...
    if (pawns.length === 0) return $list.html('<div class="empty-state"><p>Belum ada data agunan/gadai.</p></div>');
    ...
    pawns.forEach(pawn => {
        ...
        let statusClass = pawn.status === 'redeemed' ? 'text-success' : (isOverdue ? 'text-danger' : '');
        let statusText = isOverdue ? 'JATUH TEMPO' : pawn.status.toUpperCase();
        ...
        tbody.append(`
            <tr>
                ...
                <td>
                    ${pawn.status === 'active' ? `<button class="btn btn-sm btn-success redeem-pawn-btn" data-id="${pawn.id}">Tebus</button>` : '-'}
                </td>
            </tr>
        `);
    });
...
function savePawn() {
    ...
    if (!memberId || !itemName || !(itemValue > 0) || !(loanAmount > 0) || !dueDate) return showToast('Harap lengkapi semua data agunan.', 'error');

    const newPawn = {
        id: 'PAWN' + moment().format('x'), memberId, itemName, itemValue, loanAmount, dueDate,
        notes: $('#pawnNotes').val(), status: 'active', pawnDate: new Date().toISOString()
    };
...
function loadTransactionHistory(searchTerm = '') {
    ...
    if (filteredTransactions.length === 0) return $results.html('<div class="empty-state"><p>Tidak ada transaksi ditemukan.</p></div>');
    ...
    const tbody = table.find('tbody');
    filteredTransactions.slice(0, 200).forEach(t => {
        const member = members.find(m => m.id === t.memberId);
...
function generateRecapReport() {
    ...
    if (period === 'this_month') { startDate = moment().startOf('month'); endDate = moment().endOf('month'); }
    else if (period === 'last_month') { startDate = moment().subtract(1, 'months').startOf('month'); endDate = moment().subtract(1, 'months').endOf('month'); }
    else if (period === 'this_year') { startDate = moment().startOf('year'); endDate = moment().endOf('month'); }
    else { startDate = moment($('#recapStartDate').val()); endDate = moment($('#recapEndDate').val()).endOf('day'); }
    ...
    const filteredTransactions = transactions.filter(t => moment(t.date).isBetween(startDate, endDate, null, '[]'));
    const loansInPeriod = loans.filter(l => moment(l.applicationDate).isBetween(startDate, endDate, null, '[]'));
    ...
    if (activeTab === 'profitLossRecap') {
        ...
        loans.forEach(loan => {
            const paymentsInPeriod = loan.payments.filter(p => moment(p.date).isBetween(startDate, endDate, null, '[]'));
...
function openMemberProfileModal(memberId) {
    ...
    const loansHtml = memberLoans.length > 0 ? memberLoans.map(l => `<div class="summary-row"><span>${l.id} (<strong class="${l.status === 'overdue' ? 'text-danger' : (l.status === 'paid_off' ? 'text-success' : '')}">${l.status.toUpperCase()}</strong>)</span><span>Sisa: ${formatCurrency(l.outstandingPrincipal)}</span></div>`).join('') : '<p>Tidak ada pinjaman aktif.</p>';
...
// --- UTILITY FUNCTIONS ---
function updateKoperasiInfo() { $('#koperasiNameHeader').text(settings.koperasiName); document.title = `${settings.koperasiName} - Sistem Keuangan Digital`; }
function saveKoperasiInfo() { settings.koperasiName = $('#koperasiName').val(); settings.koperasiAddress = $('#koperasiAddress').val(); settings.koperasiPhone = $('#koperasiPhone').val(); settings.koperasiEmail = $('#koperasiEmail').val(); saveData('settings'); updateKoperasiInfo(); showToast('Informasi koperasi berhasil disimpan', 'success'); $('#manageKoperasiInfoModal').hide(); }
...
function showToast(message, type = 'info', duration = 3000) { const $toast = $('#toast'); $toast.removeClass('success error warning info').addClass(type); $('#toast-message').text(message); $toast.addClass('show'); setTimeout(() => $toast.removeClass('show'), duration); }
function showConfirmation(title, message, type = 'success', onConfirm) { $('#confirmTitle').text(title); $('#confirmMessage').text(message); const $confirmBtn = $('#confirmAction'); $confirmBtn.off('click').on('click', () => { onConfirm(); $('#confirmationModal').hide(); }); $confirmBtn.removeClass('btn-success btn-danger btn-warning').addClass(`btn-${type}`); $('#confirmationModal').css('display', 'flex'); }
function switchModalTab($modal, tabId) { $modal.find('.tabs .tab').removeClass('active'); $modal.find(`.tabs .tab[data-tab="${tabId}"]`).addClass('active'); $modal.find('.tab-content').removeClass('active').hide(); $modal.find(`#${tabId}Tab, #${tabId}`).addClass('active').show(); }
function logTransaction(memberId, description, amount, refId = '') {
    transactions.unshift({
        id: 'TRX' + moment().format('x'),
        date: new Date().toISOString(),
        memberId, description, amount, refId
    });
...
function getPermissionStructure() { return { "Manajemen Keuangan": { canManageSavings: "Kelola Simpanan (Setor/Tarik)", canManageLoans: "Kelola Pinjaman (Buat/Lihat)", canReceiveInstallments: "Menerima Angsuran", canManagePawns: "Kelola Agunan (Gadai)" }, "Manajemen Data": { canManageMembers: "Kelola Anggota", canManageKoperasiInfo: "Kelola Info Koperasi", canViewReports: "Lihat Laporan Keuangan" }, "Administrasi Sistem": { canAccessControlPanel: "Akses Kontrol Panel (Pengguna, Keamanan)", canBackupRestore: "Backup & Restore Data", canResetData: "Reset Semua Data" } }; }
function getPermissionKeys(isFullAccess = false) { const permissions = {}; Object.values(getPermissionStructure()).forEach(group => Object.keys(group).forEach(key => permissions[key] = isFullAccess)); return permissions; }
function checkPermission(permissionName) { if (!currentUser) return false; if (currentUser.role === 'admin') return true; return settings.roles[currentUser.role]?.permissions?.[permissionName] || false; }

function loadUsersList() {const $list = $('#usersList').empty();const table = $('<table class="data-table"><thead><tr><th>Nama</th><th>Username</th><th>Role</th><th>Aksi</th></tr></thead><tbody></tbody></table>');const tbody = table.find('tbody');users.forEach(user => {tbody.append(`<tr><td>${user.name}</td><td>${user.username}</td><td>${settings.roles[user.role]?.label || user.role}</td><td>${user.id !== DEFAULT_ADMIN.id ? `<button class="btn btn-sm btn-info edit-user" data-id="${user.id}">Edit</button> <button class="btn btn-sm btn-danger delete-user" data-id="${user.id}">Hapus</button>` : 'Default Admin'}</td></tr>`);});$list.append(table);}
function resetUserForm() {$('#userForm')[0].reset();$('#userIdInput').val('');$('#userFormTitle').text('Tambah Pengguna');}
function editUser(id) {const user = users.find(u => u.id === id);if (!user) return;resetUserForm();$('#userFormTitle').text('Edit Pengguna');$('#userIdInput').val(user.id);$('#userName').val(user.name);$('#userUsername').val(user.username).prop('disabled', true);$('#userRole').val(user.role);$('#userFormContainer').slideDown();}
function saveUser(e) {e.preventDefault();const id = $('#userIdInput').val() || 'U' + Date.now();const password = $('#userPassword').val();const existingUser = users.find(u => u.id === id);if (existingUser) { if (password) existingUser.password = CryptoJS.SHA256(password).toString(); existingUser.name = $('#userName').val(); existingUser.role = $('#userRole').val(); showToast('Data pengguna diperbarui', 'success');} else { if(!password) return showToast('Password wajib diisi untuk pengguna baru.', 'error'); const newUserData = {id,name: $('#userName').val(),username: $('#userUsername').val(),role: $('#userRole').val(), password: CryptoJS.SHA256(password).toString()}; users.push(newUserData); showToast('Pengguna baru ditambahkan', 'success');}saveData('users');loadUsersList();resetUserForm();$('#userUsername').prop('disabled', false);$('#userFormContainer').slideUp();}
function deleteUser(id) {if(users.length <= 1) return showToast('Tidak dapat menghapus satu-satunya pengguna.', 'error'); if(id === currentUser.id) return showToast('Tidak dapat menghapus akun Anda sendiri.', 'error');promptForPassword('panel', () => {showConfirmation('Hapus Pengguna', 'Yakin ingin menghapus pengguna ini?', 'danger', () => {users = users.filter(u => u.id !== id);saveData('users');loadUsersList();showToast('Pengguna dihapus', 'success');});});}
function changeSecurityCode(e) {e.preventDefault();const newCode = $('#panelSecurityCode').val();if(newCode.length < 6) return showToast('Password keamanan minimal 6 karakter.', 'error');settings.panelSecurityCode = CryptoJS.SHA256(newCode).toString();saveData('settings');showToast('Password keamanan berhasil diubah.', 'success');$(e.target).find('input').val('');}
function changeOwnPassword(e) {e.preventDefault();const currentPwd = $('#currentPassword').val();const newPwd = $('#newPassword').val();if(newPwd.length < 6) return showToast('Password baru minimal 6 karakter.', 'error');if (CryptoJS.SHA256(currentPwd).toString() !== currentUser.password) return showToast('Password saat ini salah.', 'error');const userIndex = users.findIndex(u => u.id === currentUser.id);users[userIndex].password = CryptoJS.SHA256(newPwd).toString();currentUser.password = users[userIndex].password;sessionStorage.setItem('coop_current_user', JSON.stringify(currentUser));saveData('users');showToast('Password Anda berhasil diubah.', 'success');$('#changeOwnPasswordForm')[0].reset();}
function openRoleManagementModal() {const $tabs = $('#roleTabsContainer').empty();const $permissions = $('#rolePermissionsContainer').empty();Object.entries(settings.roles).forEach(([role, data], index) => {$tabs.append(`<div class="tab ${index === 0 ? 'active' : ''}" data-tab="${role}">${data.label}</div>`);});loadRolePermissions(Object.keys(settings.roles)[0]);$('#roleTabsContainer').off('click').on('click', '.tab', function() {$('#roleTabsContainer .tab').removeClass('active');$(this).addClass('active');loadRolePermissions($(this).data('tab'));});$('#roleManagementModal').css('display', 'flex');}
function loadRolePermissions(role) {const $container = $('#rolePermissionsContainer').empty().data('current-role', role);const roleData = settings.roles[role];if (!roleData) return;const structure = getPermissionStructure();Object.entries(structure).forEach(([groupName, permissions]) => {let groupHtml = `<div class="permission-group"><div class="permission-group-title">${groupName}</div>`;Object.entries(permissions).forEach(([key, label]) => {const isChecked = roleData.permissions[key] ? 'checked' : '';const isDisabled = !roleData.isEditable ? 'disabled' : '';groupHtml += `<div class="permission-item"><label><input type="checkbox" class="permission-checkbox" data-key="${key}" ${isChecked} ${isDisabled}> ${label}</label></div>`;});groupHtml += '</div>';$container.append(groupHtml);});}
function saveRolePermissions() {const role = $('#rolePermissionsContainer').data('current-role');if (!settings.roles[role] || !settings.roles[role].isEditable) return showToast('Role ini tidak dapat diubah.', 'error');$('.permission-checkbox').each(function() {const key = $(this).data('key');settings.roles[role].permissions[key] = $(this).is(':checked');});saveData('settings');showToast(`Hak akses untuk ${settings.roles[role].label} berhasil disimpan.`, 'success');}
function populateRolesDropdown(selector) { const $select = $(selector).empty(); Object.entries(settings.roles).forEach(([key, val]) => $select.append(`<option value="${key}">${val.label}</option>`));}
function promptForPassword(type, callback) { passwordPromptCallback = callback; const title = type === 'panel' ? 'Akses Terbatas' : ''; $('#passwordPromptTitle').text(title); $('#passwordPromptMessage').text('Masukkan password keamanan untuk melanjutkan.'); $('#passwordPromptInput').val('').focus(); $('#passwordPromptModal').css('display', 'flex'); }
function submitPasswordPrompt() { const password = $('#passwordPromptInput').val(); const correctHash = settings.panelSecurityCode; if (CryptoJS.SHA256(password).toString() === correctHash) { if (passwordPromptCallback) passwordPromptCallback(); passwordPromptCallback=null; $('#passwordPromptModal').hide(); } else { showToast('Password keamanan salah.', 'error'); } }

// Data Backup, Restore, Reset
function backupData() {
    try {
        const dataToBackup = {};
        Object.keys(STORAGE_KEYS).forEach(key => { dataToBackup[STORAGE_KEYS[key]] = JSON.parse(localStorage.getItem(STORAGE_KEYS[key])); });
        const encryptedData = CryptoJS.AES.encrypt(JSON.stringify(dataToBackup), ENCRYPTION_KEY).toString();
    ...
    reader.onload = function(e) {
        showConfirmation('Restore Data', 'Ini akan menimpa semua data saat ini. Lanjutkan?', 'danger', () => {
            try {
                ...
                if(!decrypted) throw new Error("Decryption failed. Invalid key or corrupt file.");
                const restoredData = JSON.parse(decrypted);
                Object.keys(restoredData).forEach(key => { localStorage.setItem(key, JSON.stringify(restoredData[key])); });
...
function resetAllData() { showConfirmation('RESET SEMUA DATA', 'ANDA YAKIN? Tindakan ini tidak dapat diurungkan dan akan menghapus semua data anggota, transaksi, dan pengaturan.', 'danger', () => { Object.values(STORAGE_KEYS).forEach(key => localStorage.removeItem(key)); sessionStorage.clear(); showToast('Semua data telah direset. Aplikasi akan dimulai ulang.', 'success'); setTimeout(() => location.reload(), 2000); }); }

// --- PDF, Print & Excel Export ---
...
function printElement(elementId) { const content = $(elementId).clone(); $('#print-container').html(content).show(); setTimeout(() => { window.print(); $('#print-container').empty().hide(); }, 250); }
function openKtaModal(memberId) { const member = members.find(m => m.id === memberId); if(!member) return; $('#ktaKoperasiName').text(settings.koperasiName); $('#ktaKoperasiContact').text(`${settings.koperasiAddress}, ${settings.koperasiPhone}`); $('#ktaMemberName').text(member.name); $('#ktaMemberId').text(`ID: ${member.id}`); const qrContainer = document.getElementById('ktaQrcodeImg'); qrContainer.innerHTML = ''; new QRCode(qrContainer, { text: JSON.stringify({id: member.id, name: member.name}), width: 150, height: 150, colorDark : "#000000", colorLight : "#ffffff", correctLevel : QRCode.CorrectLevel.H }); $('#ktaModal').css('display', 'flex'); }
function downloadKtaAsPdf() { downloadPdf('#ktaCard', `KTA-${$('#ktaMemberName').text().replace(/ /g,"_")}.pdf`); }
function downloadRecapAsPdf() { downloadPdf('#recapReportResults .printable-area', `Laporan-Keuangan-${moment().format('YYYY-MM-DD')}.pdf`); }
...
function exportMembersToExcel() {
    const dataToExport = members.map(m => ({
        'ID Anggota': m.id, 'Nama': m.name, 'Telepon': m.phone, 'Alamat': m.address,
        'Tanggal Bergabung': moment(m.joinDate).format('YYYY-MM-DD')
    }));
    exportToExcel(dataToExport, 'Daftar_Anggota');
}
function exportLoansToExcel() {
    const dataToExport = loans.map(l => {
        const member = members.find(m => m.id === l.memberId);
        return {
            'ID Pinjaman': l.id, 'Nama Anggota': member ? member.name : 'N/A', 'Jumlah Pinjaman': l.amount,
            'Sisa Pokok': l.outstandingPrincipal, 'Tenor (Bulan)': l.tenor, 'Bunga (%)': l.interestRate,
            'Angsuran / Bulan': l.monthlyInstallment, 'Status': l.status.toUpperCase(),
            'Tanggal Pengajuan': moment(l.applicationDate).format('YYYY-MM-DD')
        };
    });
    exportToExcel(dataToExport, 'Daftar_Pinjaman');
}
function exportPawnsToExcel() {
    const dataToExport = pawns.map(p => {
        const member = members.find(m => m.id === p.memberId);
        return {
            'ID Gadai': p.id, 'Nama Anggota': member ? member.name : 'N/A', 'Nama Barang': p.itemName,
            'Nilai Taksiran': p.itemValue, 'Jumlah Pinjaman': p.loanAmount, 'Status': p.status.toUpperCase(),
    ...
    exportToExcel(dataToExport, 'Daftar_Agunan_Gadai');
}
function exportHistoryToExcel() {
    const dataToExport = transactions.map(t => {
        const member = members.find(m => m.id === t.memberId);
        return {
            'ID Transaksi': t.id, 'Tanggal': moment(t.date).format('YYYY-MM-DD HH:mm:ss'),
            'Nama Anggota': member ? member.name : 'N/A', 'ID Anggota': t.memberId,
            'Deskripsi': t.description, 'Jumlah': t.amount, 'ID Referensi': t.refId
        };
    });
    exportToExcel(dataToExport, 'Histori_Transaksi');
}
