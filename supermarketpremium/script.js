...
$(document).ready(function() {
    ...
    $('#stockInForm').submit(addIncomingStock);
    ...
    // Smart search for stock management
    ...
    // Event Listeners untuk Fitur Shift & Laporan Cerdas
...
function updateOrder() {
    ...
    // Kalkulasi Diskon Item 2%
    currentOrder.itemDiscount = 0;
    currentOrder.items.forEach(item => {
        if (item.product.hasItemDiscount) {
            currentOrder.itemDiscount += (item.product.price * item.quantity) * 0.02;
        }
    });
...
function generateProfitLossRecapHTML(startDate, endDate) {
    ...
    let totalItemDiscount = 0, totalMemberDiscount = 0;
...
function generateInventoryRecapHTML(startDate, endDate) {
    ...
    returnsIn.forEach(r => {
        r.items.forEach(item => {
            ...
            totalReturnValue += item.quantity * (product?.costPrice || 0);
        });
    });
...
function saveProduct() {
    const id = $('#productId').val(), name = $('#productName').val().trim(), category = $('#productCategory').val(),
          ...
          hasItemDiscount = $('#productHasItemDiscount').is(':checked'),
    ...
    const action = () => {
        if (id) {
            ...
            if (index > -1) products[index] = { 
                ...products[index], name, category, unit, costPrice, price, image, description, hasItemDiscount
            }; 
        } else {
            const newProduct = { 
                id: 'P' + moment().format('x'), name, category, unit, costPrice, price, stock, image, description, hasItemDiscount
            }; 
...
function loadProductStockDetails() {
    ...
    if (product) {
        ...
        $('#stockInNewCost').val(product.costPrice || 0);
        $('#stockInNewPrice').val(product.price);

        const discountStatus = product.hasItemDiscount ? 'Aktif (2%)' : 'Non-Aktif';
        $('#stockDetailsContainer').find('#stockInDiscountStatus').remove();
...
function addIncomingStock(e) {
    ...
    const newCostPrice = parseFloat($('#stockInNewCost').val());
    const newSellingPrice = parseFloat($('#stockInNewPrice').val());
    ...
    if(isNaN(newCostPrice) || newCostPrice < 0) return showToast('Harga pokok baru tidak valid.', 'error');
...
function loadProductsForStockOpname(searchTerm = '') {
    ...
    $('.physical-stock').on('input', function() {
        ...
        const loss = -diff * (product.costPrice || 0);
...
function generateReport() {
    ...
    if (reportType === 'salesReport') {
        ...
        filteredOrders.forEach(order => {
            ...
            totalDiscount += (order.discount || 0) + (order.itemDiscount || 0);
...
function openManageStoreInfoModal() {
    $('#storeName').val(settings.storeName).prop('disabled', true);
    $('#partnerName').val(settings.partnerName || '');
...
function saveStoreInfo() {
    settings.partnerName = $('#partnerName').val().trim();
