// --- PENYESUAIAN SCRIPT UNTUK LOGIN BARU ---
let captchaText = '';
let currentReportData = {}; // To store generated report data
let monthlyStatsChart, roomStatusChart;
...
function checkLoginState() {
    if (sessionStorage.getItem('isLoggedIn') === 'true') {
        ...
        generateCaptcha(); // Generate first captcha
    }
}

function handleLogin(e) {
    e.preventDefault();
    const frontOfficeName = document.getElementById('frontOfficeName').value;
    const captchaInput = document.getElementById('captchaInput').value;
    const errorEl = document.getElementById('loginError');

    if (!frontOfficeName.trim()) {
        errorEl.textContent = 'Nama Front Office wajib diisi.';
    ...
    if (captchaInput.toLowerCase() === captchaText.toLowerCase()) {
        ...
        sessionStorage.setItem('frontOfficeName', frontOfficeName);
...
function handleLogout() {
    ...
    sessionStorage.removeItem('frontOfficeName');
...
function initializeAppCore() {
    ...
    const frontOfficeName = sessionStorage.getItem('frontOfficeName');
    document.getElementById('loggedInUser').textContent = frontOfficeName;
    showNotification('Akses Diterima', `Sesi login Anda sebagai ${frontOfficeName} telah berhasil diautentikasi.`, 'success');
}

// --- SISA SCRIPT TETAP SAMA, FUNGSI INTI TIDAK DIUBAH ---
...
function setupEventListeners() {
    ...
    reservationCheckin.min = today;
    reservationCheckin.addEventListener('change', () => {
        if(reservationCheckin.value) {
            let nextDay = new Date(reservationCheckin.value);
            nextDay.setDate(nextDay.getDate() + 1);
            reservationCheckout.min = nextDay.toISOString().split("T")[0];
            if(reservationCheckout.value && reservationCheckout.value < reservationCheckin.value) {
...
function populateInvoiceModal(reservation) {
    ...
    const rooms = JSON.parse(localStorage.getItem('hotel_rooms') || '[]');
    const settings = JSON.parse(localStorage.getItem('hotel_settings') || '{}');
    const frontOfficeName = sessionStorage.getItem('frontOfficeName') || 'Staff';
...
function initDashboardCharts() {
    ...
    // Hanya hitung pendapatan dari check-out yang sudah dibayar
    reservations.forEach(res => {
        if (res.status === 'checked-out' && res.actualCheckout && res.paymentStatus === 'paid') {
            const month = new Date(res.actualCheckout).getMonth();
            monthlyRevenue[month] += res.totalAmount || 0;
        }
    });

    // Buat chart yang lebih sederhana hanya dengan satu dataset
    monthlyStatsChart = new Chart(monthlyCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: monthlyRevenue,
                backgroundColor: 'rgba(26, 94, 99, 0.7)',
