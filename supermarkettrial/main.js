...
let captchaText = '';
let premiumNotifInterval; // [BARU] Timer untuk notifikasi
...
function initializeApp() {
    ...
    setupPremiumNotification(); // [BARU] Panggil fungsi notifikasi
}
...
function handleLogout(e) {
    ...
    if (premiumNotifInterval) clearInterval(premiumNotifInterval); // [BARU] Hapus timer notifikasi
...
function generateReport() {
    ...
    if (period === 'today') { startDate = moment().startOf('day'); endDate = moment().endOf('day'); }
    else if (period === 'week') { startDate = moment().startOf('week'); endDate = moment().endOf('auto'); }
...
// --- [BARU] FUNGSI NOTIFIKASI PREMIUM ---

function setupPremiumNotification() {
    // Hapus interval lama jika ada (misal. saat re-login tanpa refresh)
    if (premiumNotifInterval) clearInterval(premiumNotifInterval);

    // Tampilkan notifikasi 10 detik setelah login/memuat ulang
    setTimeout(showPremiumNotification, 10000); 

    // Set interval untuk 1 jam
    const ONE_HOUR = 3600 * 1000;
    premiumNotifInterval = setInterval(showPremiumNotification, ONE_HOUR);
}

function showPremiumNotification() {
    const container = $('#premium-notification-container');
    if (container.is(':visible')) return; // Jangan tampilkan jika sudah ada

    fetch('notifikasi.html') // Memuat konten dari file eksternal
        .then(response => {
            if (!response.ok) throw new Error('notifikasi.html tidak ditemukan');
            return response.text();
        })
        .then(html => {
            container.html(html).show();
            
            // Pasang listener untuk tombol close di dalam HTML yang di-load
            container.off('click', '.premium-notif-close').on('click', '.premium-notif-close', function() {
                closePremiumNotification();
            });
            
            // Set timer untuk auto-close (10 detik)
            const TEN_SECONDS = 10000;
            setTimeout(closePremiumNotification, TEN_SECONDS);
        })
        .catch(err => {
            console.error('Gagal memuat notifikasi premium:', err);
        });
}
...
function closePremiumNotification() {
    ...
    container.addClass('premium-notif-hide');
    
    // Tunggu animasi selesai sebelum clear content dan hide
    setTimeout(() => {
        container.removeClass('premium-notif-hide').hide().empty();
    }, 500); // 500ms (sesuai durasi animasi)
}

// --- AKHIR DARI FUNGSI NOTIFIKASI PREMIUM ---
