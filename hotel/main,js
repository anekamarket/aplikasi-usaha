...
let roomStatusChart; // Chart untuk status kamar
...
function initApp() {
    initUsers(); // <--- PENTING: buat user default SEBELUM login

    setupPasswordToggle(); // Pasang toggle password

    // Pasang event listener login
...
function loadRooms() {
    ...
    if (editReservationRoom) {
        editReservationRoom.innerHTML = '<option value="">Pilih Kamar</option>';
        rooms.forEach(room => {
            const option = document.createElement('option');
            option.value = room.id;
            option.textContent = `${room.number} (${room.type})`;
        ...
        if (res.status === 'checked-out') {
            if (historyList) {
                ...
                historyRow.innerHTML = `
                    ...
                    <td>${formatCurrency(res.totalAmount || 0)}</td>
...
function saveReservation() {
    ...
    const newReservation = {
        ...
        guests: parseInt(document.getElementById('reservationGuests').value) || 1,
...
function editReservation(id) {
    ...
    document.getElementById('editReservationGuests').value = reservation.guests || 1;
...
function updateReservation() {
    ...
    reservations[index] = {
        ...
        checkinDate: checkinDate,
        checkoutDate: checkoutDate,
        guests: parseInt(document.getElementById('editReservationGuests').value) || 1,
        status: newStatus,
...
function editGuest(id) {
    ...
    document.getElementById('editGuestNationality').value = guest.nationality || '';
    document.getElementById('editGuestIdType').value = guest.idType || 'KTP';
